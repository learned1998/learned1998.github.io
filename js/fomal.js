...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | Learned🥝")[0];
}
...
function createtime1() {
  var grt = new Date("02/03/2025 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Learned🥝 %c 你正在访问Learned🥝の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Learned🥝") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Learned🥝的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("02/03/2025 00:00:00");
